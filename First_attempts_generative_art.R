...
for(i in 1:(boxdim1-1)){
    for(j in 1:(boxdim1-1)){
      newrojo[i,j] = mean(rojo[(i-1):(i+1), (j-1):(j+1)])
    }
  newrojo[i,j+1] = mean(rojo[(i-1):(i+1), j:(boxdim1)])
}
for(j in 1:(boxdim1-1)){
  newrojo[boxdim1,j] = mean(rojo[(boxdim1-1):(boxdim1), j-1:j+1])
}
newrojo[i+1,j+1] = mean(rojo[(boxdim1-1):boxdim1, boxdim1-1:boxdim1])
rojo = newrojo
}
return(rojo)
}
...
rojo = spreader(100, 0.1, 10)
verde=  spreader(100, 0.9, 10)
azul =  spreader(100, 0.3, 2)
